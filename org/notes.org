...
- [ ] Finish the 'dot plot' example from the DataCamp review
- [ ] Quiz 2 is live - complete it by Friday September 6, 11:59 PM
- [ ] Enter project team & idea in Canvas ([[https://lyon.instructure.com/courses/2629/pages][wiki]]) by Friday 11 AM
- [ ] Continue lecture + codealong (EDA)
...
9. Exercise!
   ...
   - Close your R console.

   - Go to your home directory (=C-x d ~/=)

   - Make a new directory in the =Dired= buffer with =+ org=

   - Find the directory and go into it

   - In =~/org=, create a file =test.org=

   - In =test.org=, open a graphic R code block

   - Set the file header argument so that you get a PNG file

   - Create a sorted dot plot of the length of the guinea pig teeth
     from the =ToothGrowth= dataset:
     #+begin_example R
     dotchart(sort(ToothGrowth$len))
     #+end_example

   - When asked for the "R starting project directory", modify the
     offered address and change it from =~/org= to =~/=

   - Open the file with =<F6>=. You shouldn't see anything

   - Split the screen and open the R console in one, and the file
     =test.org= in the other buffer

   - You should see that =*R*= is set to your =$HOME=.

   - Create another (non-graphic) R code block (keep =*R*= open)

   - In the other code block check where you are - both with an R
     function, and with a =system= command

   - You know that your test file is in =~/org=. Now set the working
     directory to that directory using a /relative path/, and check the
     new location.

   - Re-run the graphics command from earlier and open the PNG file
     with =<F6>=.

   - Make sure that you understand everything we're doing here! Create
     your own examples!
...
- [ ] Use =ToothGrowth=: color the dot plot by =supp= values
