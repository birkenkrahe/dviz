...
* Review questions "Visualizing two variables"
...
5. When should you use a line plot? What's an example?
   ...
   An example would be any plot of observations over time (also called
   a 'time series'). Line plots can be overlayed easily.
   #+end_quote   
   Example:
...
6. Which examples should you pick when educating people about data
   science? Should the data be (easily) validatable or not? Examples?
   #+begin_quote
   - Examples for contested data: growth of COVID-19 cases world-wide,
     wealth of nations, gun murder data, data about abortions.
   - Example for uncontested data: growth of world population, change
     in baby names, water flow through the Nile, number of births or
     deaths.
   #+end_quote
