...
* Week 3 - Linux Party & Introduction to Visualization
...
- In Emacs, you can look up variables with =C-h v= - for example =C-h v
  org-babel-C-compiler= (its value is gcc)

- Look up the variable =org-structure-template-alist=. In the buffer
  that opens, move the cursor to =customize= and click the link,

- In the next buffer, you see how =<s= works (for a =src= code block), and
  also =<e= (for an =example= block).

- Move to the end of the page (=M->=) then click on =INS=, and then enter:
  #+begin_example
  Key: r
...
- Enter another shorthand for graphics:
  #+begin_example
  Key: rg
...
- In the same way you could add a short code for the =#+PROPERTY:= line
  if you wanted to.

- Before leaving this buffer, go back up (=M-<=) and click on =Apply and
  Save= - this will add the customization to your =.emacs= configuration
  file.

- The other useful shorthand is the =#+startup= meta data at the top of
  each file. This is run by another variable, =org-tempo-keywords-alist=.

- Open it's menu with =C-h v org-tempo-keywords-alist=.

- Again, find and click on =customize=.

- In the customization buffer, Go to the end of the list and insert with =INS=:
  #+begin_example
  Key: S
  Keyword: STARTUP: overview hideblocks indent
  #+end_example

- At the top, click on =Apply and Save= to fix the setting.

- These settings should be active right away but they will certainly
