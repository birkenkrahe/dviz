...
* IN PROGRESS Review questions "Visualizing two variables" (DataCamp)
...
5. When should you use a line plot? What's an example?
   ...
   #+end_quote
   Example:
   #+begin_src R :file ts2.png :session *R* :results file graphics output :exports both
     ## First plot
     plot(Nile, ylim=range(Nile, Nile+100), ylab="Flow", main="Overlayed Plot of Nile Data")
